...
app.get('/getuser', (req, res) => {
    ...
    connection.query('SELECT * FROM albumRater.login WHERE username = ?', [user], (error, results) => {
        if (error) {
...
app.get('/newuser', (req, res) => {
    ...
    connection.query('INSERT INTO albumRater.login VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', [user, pass, null, json, 0, dateCreated, json, json, json, json, null, 0, null], (error) => {
        if (error) {
            return res.status(500).send('Error executing query: ' + error.stack);
        }
        console.log('added')
...
app.get('/getPic', (req, res) => {
    const user = req.query.user
    connection.query('SELECT pic FROM albumRater.login WHERE username = ?', [user], (error, results) => {
        if (error) {
...
app.get('/newalbum', (req, res) => {
    ...
    connection.query('UPDATE albumRater.login SET albums = ?, numAlbums = ? WHERE username = ?', [album, numAlbums, user], (error) => {
        if (error) {
            return res.status(500).send('Error executing query: ' + error.stack);
        }
        console.log('added')
...
app.get('/newListenList', (req, res) => {
    ...
    const user = req.query.user
    connection.query('UPDATE albumRater.login SET listenList = ? WHERE username = ?', [listenList, user], (error) => {
        if (error) {
            return res.status(500).send('Error executing query: ' + error.stack);
        }
        console.log('added')
...
app.get('/setBanner', (req, res) => {
    ...
    connection.query('UPDATE albumRater.login SET banner = ? WHERE username = ?', [banner, user], (error) => {
        if (error) {
            return res.status(500).send('Error executing query: ' + error.stack);
        }
        console.log('added')
...
app.get('/setBannerPos', (req, res) => {
    ...
    connection.query('UPDATE albumRater.login SET bannerPos = ? WHERE username = ?', [bannerPos, user], (error) => {
        if (error) {
            return res.status(500).send('Error executing query: ' + error.stack);
        }
...
app.get('/setRanking', (req, res) => {
    ...
    connection.query('UPDATE albumRater.login SET albums = ? WHERE username = ?', [albums, user], (error) => {
        if (error) {
            return res.status(500).send('Error executing query: ' + error.stack);
        }
        console.log('added')
...
app.post('/setProfilePic', (req, res) => {
    ...
    /* store profile pic */
    connection.query('UPDATE albumRater.login SET pic = ? WHERE username = ?', [binaryData, username], (error) => {
        if (error) {
...
app.get('/newUsername', (req, res) => {
    ...
    connection.query('UPDATE albumRater.login SET username = ? WHERE username = ?', [newuser, olduser], (error) => {
        if (error) {
            return res.status(500).send('Error executing query: ' + error.stack);
        }
...
app.get('/newPassword', (req, res) => {
    ...
    connection.query('UPDATE albumRater.login SET password = ? WHERE username = ?', [newPass, user], (error) => {
        if (error) {
            return res.status(500).send('Error executing query: ' + error.stack);
        }
...
app.get('/deleteAccount', (req, res) => {
    const user = req.query.user
    connection.query('DELETE FROM albumRater.login WHERE username = ?', [user], (error) => {
        if (error) {
            return res.status(500).send('Error executing query: ' + error.stack);
        }
...
app.get('/getUsers', (req, res) => {
    connection.query('SELECT * FROM albumRater.login', (error, results) => {
        if (error) {
            return res.status(500).send('Error executing query: ' + error.stack);
        }
        res.json(results);
